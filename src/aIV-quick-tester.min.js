...
/* Algorithm IV Quick Tester (v0.0.1) (learn@algorithmiv.com)
 * Section: The Main JavaScript Module
 * Author: Adam Smith (adamsmith@youlum.com)
 * Copyright (c) 2015 Adam A Smith (github.com/imaginate)
 * The Apache License (algorithmiv.com/docs/license) */
(function(b,p){window.aIV=window.aIV||{};aIV.tester={};aIV.tester.runTests=b.runTests;aIV.tester.addTest=b.addTest})(function(b){function p(){e.ui.style.opacity="0";setTimeout(function(){e.msg.innerHTML="Tests are running ... ";e.start.style.display="none";e.ui.style.opacity="1"},500)}function r(d,c){var a,g,m,b,f;a="<h2>Results</h2>";a+='<ol id="results">';m=!1;b=c+1;for(f=d-1;++f<b;)g=!!h[f].errors.length,a+=t(h[f].title,g),g&&(m=!0);a+="</ol>";if(m){a+="<h2>Errors</h2>";a+='<ol id="errors">';for(f=
d-1;++f<b;)(g=!!h[f].errors.length)&&(a+=u(h[f].title,h[f].errors));a+="</ol>"}e.ui.style.opacity="0";setTimeout(function(){e.ui.innerHTML=a;e.ui.style.opacity="1"},500)}function t(d,c){var a;return a='<li class="'+(c?"red":"green")+'">'+d+" =&gt; "+(c?"Fail":"Pass")+"</li>"}function u(d,c){var a,g,b;a="<li>"+d+'<ol id="subErrors">';g=c.length;for(b=-1;++b<g;)a+="<li>"+c[b]+"</li>";return a+"</ol></li>"}b=aIV.utils.getElemById;var k=aIV.utils.checkType,q=!1,l=0,n=[],h=[],e={msg:b("msg"),ui:b("ui"),
start:b("start")};return{runTests:function(d,c){var a,b;if(q)throw Error("aIV.tester.runTests was called a second time.");q=!0;p();b=n.length;if(!k(d,"number")||0>d)d=0;if(!k(c,"number")||0>c)c=b-1;if(c>=b)throw Error("aIV.tester.runTests was called with an invalid endIndex.");if(d>=b||d>c)throw Error("aIV.tester.runTests was called with an invalid startIndex.");b=c+1;for(l=d-1;++l<b;)try{n[l]()}catch(e){a=e.name+": "+e.message,h[l].errors.push(a),a+="; STACK TRACE: %O",console.error(a,e.stack)}r(d,
c)},addTest:function(b,c){if(!k(b,"string"))throw new TypeError("aIV.tester.addTest was called with an invalid title.");if(!k(c,"function"))throw new TypeError("aIV.tester.addTest was called with an invalid testFunc.");n.push(c);h.push({title:b,errors:[]})}}}());
